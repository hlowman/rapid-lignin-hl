...
# Rationale for sigma 8, lambda, s/v, c/v, and bd/v is anything at or above approximately 6 sd from the mean. additional marine sigma 8 sample (56) removed due to contamination concerns.
...
bdv <- mained %>%
  dplyr::select(Group_3f, Environment, LTER_Site, Depthf, Replicate, Core, Group, `Bd/V`) %>%
  filter(!is.na(`Bd/V`)) %>%
  mutate(BDV = `Bd/V`) %>%
  replace_na(list(Depthf = 10, Replicate = 1, Core = "T"))
...
bdv$logBDV <- log10(bdv$BDV) # Makes log-transformed column.
...
ggpairs(bdv) # Quick pairs plot to check correllated variables.
...
ggplot(data = bdv, aes(x = LTER_Site, y = rf1)) +
  geom_boxplot() + 
  geom_point() +
  labs(x = "Site", y = "Standardised residuals") # Plots said residuals.
...
f3 <- lme(logBDV ~ Group_3f + Environment, 
          random =~1 | LTER_Site / Depthf / Replicate / Core, data = bdv) # Creates the first LMEM.
anova(f2, f3) # Compares the two models. f3 preferred with AIC value of 6.34489.

# STEP 3: Decide on a variance structure (aka random terms and correlation structure if any).

plot(f3, col = 1) # Looks fairly evenly distributed.
qqnorm(f3) # Choosing not to apply another variance structure.
...
f3_ml <- lme(logBDV ~ Group_3f + Environment, 
             random =~1 | LTER_Site / Depthf / Replicate / Core, 
             method = "ML", data = bdv) # Creates current model using ML structure.
f4 <- lme(logBDV ~ Environment, 
          random =~1 | LTER_Site / Depthf / Replicate / Core, 
          method = "ML", data = bdv) # Removes region.
f5 <- lme(logBDV ~ Group_3f, 
          random =~1 | LTER_Site / Depthf / Replicate / Core, 
          method = "ML", data = bdv) # Removes ecosystem.
anova(f3_ml, f4, f5) # Compares all three models. f4 is preferred with an AIC value of -11.097069. So, remove Region.

f6full <- lme(logBDV ~ Environment, 
              random =~1 | LTER_Site / Depthf / Replicate / Core, 
              method = "ML", data = bdv)
f6sub1 <- update(f6full, .~. -Environment) # Just checking...
anova(f6full, f6sub1) # Keep Environment.

# STEP 9: Refit with REML

ffinal <- lme(logBDV ~ Environment, 
              random =~1 | LTER_Site / Depthf / Replicate / Core, 
              method = "REML", data = bdv) 
# Final full model.
...
# I applied a linear mixed effect modelling approach because the data are nested. My model suggests there is a significant effect of ecosystem type (stream, estuarine, marine) on 3,5-Bd/V signatures of a given sediment sample. Random nested intercepts by site/depth/replicate/core section were also included.

# Equation: log(3,5-Bd/V) = -0.77 + 0.006[Estuary] + 0.44[Reef] + random

# BONUS : Tukey's HSD Post Hoc
fHSD <- glht(ffinal, linfct=mcp(Environment="Tukey"))
summary(fHSD) # Significantly different: Stream & Reef (p < 0.001) and Estuary & Reef (p = 0.0254)
...
The final model took the form : log(3,5-Bd/V) ~ Environment + 1|LTER_Site / Depthf / Replicate / Core

This translated to a formula of log(3,5-Bd/V) = -0.77 + 0.006[Estuary] + 0.44[Reef] + random.

Post hoc results: Stream - Estuary, p = 0.9995 (NS); Stream - Reef, p < 0.001; Estuary - Reef, p = 0.0254.
...
# Some additional grain size estimates for the manuscript.

GSmeans1 <- mained %>%
  group_by(Environment, Depthf) %>%
  summarize(mean_med = mean(`Diameter_50.00%`, na.rm = TRUE),
            sd_med = sd(`Diameter_50.00%`, na.rm = TRUE),
            mean_clay = mean(perc_Clay, na.rm = TRUE),
            sd_clay = sd(perc_Clay, na.rm = TRUE),
            mean_silt = mean(perc_Silt, na.rm = TRUE),
            sd_silt = sd(perc_Silt, na.rm = TRUE),
            mean_sand = mean(perc_Sand, na.rm = TRUE),
            sd_sand = sd(perc_Sand, na.rm = TRUE)) %>% # Calculates summaries ...
  ungroup()
